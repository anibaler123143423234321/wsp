...
// (El componente MicrophoneIcon se mantiene igual...)
...
const VoiceRecorder = ({ onSendAudio, canSendMessages, onRecordingStart, onRecordingStop }) => {
  ...
  // 🔥 1. ESTADO PARA EVITAR DOBLE ENVÍO
  ...
  const startRecording = async () => {
    // 🔥 Si ya estamos enviando, no permitir grabar nuevo
    if (isSendingLocal) return;

    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      streamRef.current = stream;
      // ... (resto de tu lógica de startRecording igual)
  ...
  const sendAudio = () => {
    // 🔥 2. BLOQUEO: Si no hay audio, tiempo 0, o YA SE ESTÁ ENVIANDO, salir.
    if (!audioBlob || recordingTime === 0 || isSendingLocal) return;

    setIsSendingLocal(true); // Bloqueamos el botón
    ...
    setRecordingTime(0);
    audioChunksRef.current = [];
    setIsSendingLocal(false); // Desbloqueamos (aunque el componente se desmonte visualmente)

    // Restauramos UI del padre
    if (onRecordingStop) onRecordingStop();
  };
  ...
  if (isRecording || audioBlob) {
    return (
      <div className="voice-recorder-active">
        <button className="voice-recorder-cancel" onClick={cancelRecording} disabled={isSendingLocal}>
          <FaTimes />
        </button>
        ...
        <div className="voice-recorder-info">
          {isRecording ? (
            <>
              <div className="voice-recorder-pulse"><MicrophoneIcon /></div>
              <span className="voice-recorder-time recording">{formatTime(recordingTime)}</span>
            </>
          ) : (
            <>
              <div className="voice-recorder-icon"><MicrophoneIcon /></div>
        ...
        {isRecording ? (
          <button className="voice-recorder-stop" onClick={stopRecording}>
            <FaStop />
          </button>
        ) : (
          // 🔥 3. Deshabilitar botón mientras envía y mostrar loading visual si quieres
          <button
            className="voice-recorder-send"
            onClick={sendAudio}
            disabled={isSendingLocal}
            style={{ opacity: isSendingLocal ? 0.7 : 1 }}
          >
            <FaPaperPlane />
          </button>
        )}
